...
public class FastTableSaw extends BasicFastMachine {

    public FastTableSaw(ItemGroup itemGroup, SlimefunItemStack itemStack, RecipeType recipeType, ItemStack[] recipe) {
        super(itemGroup, itemStack, recipeType, recipe);
    }

    @Override
    public Material getCraftItemMaterial() {
        return MaterialCompat.safe(XMaterial.STONECUTTER);
    }

    @Override
    public RecipeLoader getRecipeLoader() {
        return new SlimefunDisplayRecipeLoader(this, SlimefunItems.TABLE_SAW.getItemId());
    }
}
